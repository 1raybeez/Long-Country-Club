...
export function HomeLiveAction({ initialView, personalStanding }: { initialView: HomeCurrentSeasonView; personalStanding: (CurrentStanding & { rank: number }) | null }) {
  ...
  const statusLabel = view.matchup.postseason?.isBye ? "Bye" : view.matchup.currentStatus ? formatMatchupStatus(view.matchup.currentStatus) : null;
  const postseasonLabel = view.matchup.postseason && !view.matchup.postseason.isBye
    ? `${view.matchup.postseason.roundLabel} · ${view.matchup.postseason.bracketType === "winners" ? "Championship Bracket" : "Consolation Bracket"}`
    : null;
