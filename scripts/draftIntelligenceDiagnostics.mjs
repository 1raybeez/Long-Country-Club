...
check("normalized_opportunity", x.opportunityCosts.every((o) => o.marketRankGap >= 8 && o.alternativeConfidence === "HIGH" && o.alternativeRank < o.selectedNormalizedRank && o.selectedPosition === o.alternativePosition));
check("n24b_methodology", x.methodologyVersion === "rookie-draft-intelligence-n2.4b-commissioner-grades-v1" && x.opportunityCostAudit?.numericBasis === "SAME_POSITION_ONLY" && x.opportunityCostAudit?.crossPositionBasis === "NARRATIVE_ONLY");
check("same_position_numeric_opportunity", x.pickGrades.every((p) => !p.opportunityCostFlag || (p.strongestSupportedAlternative?.position === p.position && p.strongestSupportedAlternative?.alternativeConfidence === "HIGH" && p.strongestSupportedAlternative?.marketRankGap >= 8)));
check("one_numeric_alternative_per_pick", x.pickGrades.every((p) => !p.opportunityCostFlag || Object.keys(p).includes("strongestSupportedAlternative")) && new Set(x.opportunityCosts.map((o) => `${o.ownerId}:${o.pick}`)).size === x.opportunityCosts.length);
check("cross_position_is_narrative_only", x.pickGrades.every((p) => (p.crossPositionOpportunityAlternatives ?? []).every((a) => a.position !== p.position)));
check("signed_best_worst", x.ownerGrades.every((o) => (o.bestPick === "NO CLEAR VALUE PICK" || x.pickGrades.some((p) => p.ownerId === o.ownerId && p.player === o.bestPick && p.marketValueDelta > 0)) && (o.worstOrRiskiestPick === "NO CLEAR REACH" || x.pickGrades.some((p) => p.ownerId === o.ownerId && p.player === o.worstOrRiskiestPick && p.marketValueDelta < 0))));
check("best_worst_distinct", x.ownerGrades.every((o) => o.bestPick !== o.worstOrRiskiestPick));
check("roast_praise_lock", x.roastHooks.every((h) => ["STRONG ROAST", "LIGHT ROAST", "NO ROAST"].includes(h.confidence)));
...
console.log(JSON.stringify({ status: x.status, checks: { canonical48: true, valueDirection: true, reachDirection: true, arithmetic: true, legacySignRemoved: true, confidence: true, unrankedSafe: true, capitalAdjusted: true, opportunityDirection: true, opportunityAvailable: true, ownerAggregates: true, separateClassImpact: true, rubricProvisional: true, noFinalGrades: true, n24bMethodology: true, samePositionNumericOpportunity: true, oneNumericAlternativePerPick: true, crossPositionNarrativeOnly: true, signedBestWorst: true, bestWorstDistinct: true, roastPraiseLock: true }, ranked: ranked.length, owners: Object.keys(x.owners).length, opportunities: x.opportunityCosts.length, strongRoasts: x.roastHooks.filter((h) => h.confidence === "STRONG ROAST").length, lightRoasts: x.roastHooks.filter((h) => h.confidence === "LIGHT ROAST").length }, null, 2));
