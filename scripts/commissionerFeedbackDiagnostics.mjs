...
const outputDirectory = mkdtempSync(join(tmpdir(), 'lcc-commissioner-feedback-'));

try {
  const compile = spawnSync(localTsc, [
    'scripts/commissionerFeedbackDiagnostics.ts',
  ...
  require(join(outputDirectory, 'scripts', 'commissionerFeedbackDiagnostics.js'));
} finally {
