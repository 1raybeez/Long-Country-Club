...
import { DYNASTY_DIRECTION_MODEL_VERSION, TRADE_FIT_MODEL_VERSION, type DynastyConfidence, type DynastyDirection, type DynastyDimension, type DynastyParticipant, type DynastyProfile, type DynastyDirectionResult, type DynastyPresentation, type TradeFit } from "./dynastyDirectionTypes.ts";
...
function analyzeParticipant(input: DynastyDirectionInput): DynastyParticipant {
  ...
  return { franchiseId: input.franchiseId, franchiseName: input.franchiseName, before, after, directionChanged: before.direction !== after.direction, tradeFit: fit.fit, fitReasons: fit.reasons, changes, status, warnings, presentation: presentation(before, after, changes, fit.fit, input.sends, input.receives, warnings) };
}
...
function presentation(before: DynastyProfile, after: DynastyProfile, changes: DynastyParticipant["changes"], fit: TradeFit, sends: CurrentCatalogAsset[], receives: CurrentCatalogAsset[], warnings: string[]): DynastyPresentation {
  const winNow = changes.lineupStrength === "IMPROVED" ? "Starting lineup improves." : changes.lineupStrength === "REDUCED" ? "Starting lineup declines." : changes.lineupStrength === "UNCHANGED" ? "Starting lineup is unchanged." : "Starting lineup evidence is unavailable.";
  const careerLabel = changes.age === "YOUNGER" ? "Gets younger" : changes.age === "OLDER" ? "Gets older" : changes.age === "UNCHANGED" ? "Little change" : "Unavailable";
  const careerDetail = changes.age === "UNAVAILABLE" ? "Starter age evidence is unavailable." : before.starterAge !== null && after.starterAge !== null ? `Expected starter age: ${before.starterAge.toFixed(1)} -> ${after.starterAge.toFixed(1)} years.` : "Starter age evidence is unavailable.";
  const addedPicks = receives.filter((asset) => asset.assetType === "PICK").map((asset) => asset.displayName);
  const removedPicks = sends.filter((asset) => asset.assetType === "PICK").map((asset) => asset.displayName);
  const futureUnavailable = changes.futureCapital === "UNAVAILABLE" || warnings.includes("FUTURE_PICK_DATA_UNAVAILABLE");
  const futureLabel = futureUnavailable ? "Unavailable" : addedPicks.length || removedPicks.length ? "Changes" : "No change";
  const futureDetail = futureUnavailable ? "Future-pick evidence is unavailable." : addedPicks.length || removedPicks.length ? [addedPicks.length ? `Adds ${addedPicks.join(", ")}.` : "", removedPicks.length ? `Moves out ${removedPicks.join(", ")}.` : ""].filter(Boolean).join(" ") : "No future-pick change in this trade.";
  const distribution = before.assetDistribution.available && after.assetDistribution.available ? changes.rosterValue === "INCREASED" && changes.age === "YOUNGER" ? "Younger core value increases." : changes.rosterValue === "INCREASED" ? "Long-term asset profile improves." : changes.rosterValue === "DECREASED" && changes.age === "OLDER" ? "Veteran-heavy value increases." : changes.rosterValue === "DECREASED" ? "Long-term asset profile declines." : "Little structural change." : "Asset-distribution evidence is unavailable.";
  const direction = before.direction === after.direction ? `Remains ${after.direction}.` : `${before.direction} -> ${after.direction}.`;
  const immediateDecline = changes.lineupStrength === "REDUCED";
  const longTermGain = changes.age === "YOUNGER" || changes.futureCapital === "INCREASED" || changes.rosterValue === "INCREASED";
  const fitExplanation = before.direction === "CONTENDER" || before.direction === "PLAYOFF PUSH" ? immediateDecline && longTermGain ? "Improves the long-term asset profile but weakens a roster built to compete now." : changes.lineupStrength === "UNCHANGED" && longTermGain ? "Adds long-term flexibility without improving the current starting lineup." : fit === "POOR FIT" ? "Weakens the current competitive window without enough measured long-term compensation." : fitReasonsFor(fit) : before.direction === "REBUILDING" && immediateDecline && longTermGain ? "Sacrifices current production for younger assets and future flexibility, which matches a rebuilding direction." : fit === "FIT" && changes.lineupStrength !== "REDUCED" ? "Improves the roster without materially changing the franchise's competitive window." : fitReasonsFor(fit);
  return { winNow, careerWindow: { label: careerLabel, detail: careerDetail }, futureCapital: { label: futureLabel, detail: futureDetail, addedPicks, removedPicks }, assetProfile: { label: "Asset Profile", detail: distribution }, direction, fitExplanation };
}
function fitReasonsFor(fit: TradeFit) { return fit === "INSUFFICIENT EVIDENCE" ? "There is not enough evidence to judge the long-term fit." : fit === "MIXED" ? "Immediate and long-term signals point in different directions." : fit === "POOR FIT" ? "The measured changes do not fit the current competitive direction." : "The measured changes fit the current competitive direction."; }
function incomplete(input: DynastyDirectionInput, warnings: string[]): DynastyParticipant { const empty = (): DynastyProfile => ({ direction: "UNCLEAR", confidence: "LOW", immediateStrength: dimension(null, false, "Unavailable."), rosterStrengthDimension: dimension(null, false, "Unavailable."), ageCareerWindow: dimension(null, false, "Unavailable."), futureCapital: dimension(null, false, "Unavailable."), assetDistribution: dimension(null, false, "Unavailable."), rosterValue: null, starterAge: null, youngerCoreShare: null, veteranCoreShare: null, futurePickCount: null, futurePickValue: null, warnings }); return { franchiseId: input.franchiseId, franchiseName: input.franchiseName, before: empty(), after: empty(), directionChanged: false, tradeFit: "INSUFFICIENT EVIDENCE", fitReasons: ["Insufficient evidence to determine dynasty direction."], changes: { rosterStrength: "UNAVAILABLE", lineupStrength: "UNAVAILABLE", age: "UNAVAILABLE", futureCapital: "UNAVAILABLE", rosterValue: "UNAVAILABLE" }, status: "INCOMPLETE", warnings, presentation: { winNow: "Starting lineup evidence is unavailable.", careerWindow: { label: "Unavailable", detail: "Starter age evidence is unavailable." }, futureCapital: { label: "Unavailable", detail: "Future-pick evidence is unavailable.", addedPicks: [], removedPicks: [] }, assetProfile: { label: "Asset Profile", detail: "Asset-distribution evidence is unavailable." }, direction: "Direction is unclear.", fitExplanation: "There is not enough evidence to judge the long-term fit." } }; }
