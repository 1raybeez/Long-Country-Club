...
async function loadRuntime(): Promise<TradeAnalyzerRuntime> {
  const normalizedPath = `data/trade-analyzer/valuations/fantasycalc/normalized/${APPROVED_DATE}.json`;
  const rawPath = `data/trade-analyzer/valuations/fantasycalc/raw/${APPROVED_DATE}.json`;
  const manifestPath = "data/trade-analyzer/valuations/fantasycalc/manifest.json";
  const [snapshot, raw, manifest, roster, playerCatalog, futurePicks] = await Promise.all([readJson(normalizedPath), readJson(rawPath), readJson(manifestPath), readJson("data/current/rosters/2026.json"), readJson("data/history/matchups/sleeper/players.json"), readJson("data/current/drafts/future-picks.json")]);
  const normalizedText = await readFile(file(normalizedPath), "utf8");
